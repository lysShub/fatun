...
func Test_bpfFilterProtoAndLocalPorts(t *testing.T) {
	...
	for i, e := range suits {
		vm, err := bpf.NewVM(bpfFilterProtoAndSkipLocalTCPPorts(e.TCPSkipPorts, e.UDPSkipPorts))
		require.NoError(t, err, i)

		n, err := vm.Run(e.IP)
		require.NoError(t, err, i)
		require.Equal(t, e.Return, uint16(n), i)
	}
}
