...
type Server interface {
	MaxRecvBuffSize() int
	Logger() *slog.Logger
	AddSession(sess fatun.Session, pxy Proxyer) error
	Send(sess fatun.Session, pkt *packet.Packet) error
	Close(client netip.AddrPort)
}

type Proxyer interface {
	DelSession(fatun.Session)
	Downlink(*packet.Packet, sconn.Peer) error
	Closed() bool
}
