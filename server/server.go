...
func (s *Server) recvService(conn *net.IPConn) error {
	...
	for {
		...
		if !has {
			...
			err = p.Downlink(pkt, sconn.Peer{Remote: sess.Src.Addr(), Proto: sess.Proto})
			if err != nil {
				if !p.Closed() {
...
func (s *Server) send(sess fatun.Session, pkt *packet.Packet) error {
