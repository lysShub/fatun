...
type ttlkey struct {
	s fatun.Session // {Src: process-addr, Dst: request-addr}
	t time.Time
}
...
func (t *ttlmap) cleanup() {
	...
	t.donwlinkMu.Lock()
	for i, e := range ss {
		s := fatun.Session{Src: e.Dst, Proto: e.Proto, Dst: netip.AddrPortFrom(t.addr, lports[i])}
...
func (t *ttlmap) Add(s fatun.Session, p proxyer.Proxyer) error {
	t.cleanup()
...
func (t *ttlmap) Uplink(s fatun.Session) (localPort uint16, has bool) {
	t.uplinkMu.RLock()
...
func (t *ttlmap) Downlink(s fatun.Session) (p proxyer.Proxyer, clientPort uint16, has bool) {
	t.donwlinkMu.RLock()
